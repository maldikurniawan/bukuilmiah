...
class BukuReferensiController extends Controller
{
    ...
    {
        $update = DB::table('bukureferensi')->where('id', $id)->update([
            'status' => 1
        ]);
        if ($update) {
            return redirect(route('bukureferensi.index'))->with(['success' => 'Data Berhasil Diperbaharui']);
        } else {
            return redirect(route('bukureferensi.index'))->with(['warning' => 'Data Gagal Diperbaharui']);
        }
    }
